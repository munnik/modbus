...
func (tt *tcpTransport) readResponse() (res *pdu, err error) {
	...
	for {
		...
		var unknownProtocolIdError *UnknownProtocolIdError
		if errors.As(err, &unknownProtocolIdError) {
			continue
		}
...
func (tt *tcpTransport) readMBAPFrame() (p *pdu, txnId uint16, err error) {
	...
	if bytesNeeded+mbapHeaderLength > maxTCPFrameLength {
		err = NewProtocolError(" read more than the max allowed frame length")
		return
	}

	// an MBAP length of 0 is illegal
	if bytesNeeded <= 0 {
		err = NewProtocolError("an MBAP length of 0 is illegal")
		return
	}
	...
	if protocolId != 0x0000 {
		err = NewUnknownProtocolIdError("received unexpected protocol id 0x%04x", protocolId)
		return
	}
