...
func NewServer(conf *ServerConfiguration, reqHandler RequestHandler) (
	...
	if ms.conf.URL == "" {
		err = &ConfigurationError{msg: fmt.Sprintf("missing host part in URL '%s'", conf.URL)}
		return
	}
	...
	case "tcp+tls":
		...
		if ms.conf.TLSServerCert == nil {
			err = NewConfigurationError("missing server certificate")
			return
		}
		...
		if ms.conf.TLSClientCAs == nil {
			err = NewConfigurationError("missing CA/client certificates")
			return
		}
	...
	default:
		err = NewConfigurationError("unkown server type %s", serverType)
		return
	}
...
func (ms *ModbusServer) Start() (err error) {
	...
	default:
		err = NewConfigurationError("unknown transport type %v", ms.transportType)
		return
	}
	...
	for {
		...
		case fcReadCoils, fcReadDiscreteInputs:
			...
			if len(req.payload) != 4 {
				err = NewProtocolError("length of request payload (%d) is not 4", len(req.payload))
				break
			}
			...
			if quantity > 2000 || quantity == 0 {
				err = NewProtocolError("amount of registers to read (%d) is not supported", quantity)
				break
			}
			if uint32(addr)+uint32(quantity)-1 > 0xffff {
				err = NewIllegalDataAddressError("trying to read past 0xffff")
				break
			}
			...
			if err == nil && resCount != int(quantity) {
				err = NewServerDeviceFailureError("handler returned %v bools, expected %v", resCount, quantity)
				break
			}
		...
		case fcWriteSingleCoil:
			if len(req.payload) != 4 {
				err = NewProtocolError("the length of the payload (%d) is not the expected length (%d)", len(res.payload), 4)
				break
			}
			...
			if (req.payload[2] != 0xff && req.payload[2] != 0x00) || req.payload[3] != 0x00 {
				err = NewProtocolError("unexpected value (%v) for payload, should be either 0xff00 or 0x0000 and 0x0000", req.payload)
				break
			}
		...
		case fcWriteMultipleCoils:
			...
			if len(req.payload) < 6 {
				err = NewProtocolError("the length of the payload (%d) is less than expected minimum length (%d)", len(res.payload), 6)
				break
			}
			...
			if req.payload[4] != uint8(expectedLen) {
				err = NewProtocolError("the byte count field of the payload (%d) doesn't equal the expected length (%d)", res.payload[4], uint8(expectedLen))
				break
			}

			// make sure we have enough bytes
			if len(req.payload)-5 != expectedLen {
				err = NewProtocolError("the length of the payload (%d) is not the expected length (%d)", len(req.payload)-5, expectedLen)
				break
			}
		...
		case fcReadHoldingRegisters, fcReadInputRegisters:
			...
			if len(req.payload) != 4 {
				err = NewProtocolError("the length of the payload (%d) is not the expected length (%d)", len(req.payload), 4)
				break
			}
			...
			// make sure the handler returned the expected number of items
			if err == nil && resCount != int(quantity) {
				err = NewServerDeviceFailureError("handler returned %v 16-bit values, expected %v", resCount, quantity)
				break
			}
		...
		// and log an error
		if err == nil && res == nil {
			err = NewServerDeviceFailureError("internal server error (req: %v, res: %v, err: %v)", req, res, err)
		}
		...
		if err != nil {
			...
			if errors.As(err, &protocolError) {
				...
				t.Close()
				return
			} else if excpetionCodeError, ok := err.(ExceptionCodeError); ok {
				res = &pdu{
					unitId:       req.unitId,
					functionCode: (0x80 | req.functionCode),
					payload:      []byte{excpetionCodeError.GetExceptionCode()},
				}
			} else {
				ms.logger.Fatalf("unexpected error, %s", err)
			}
		}
