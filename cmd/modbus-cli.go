...
func performBoolScan(client *modbus.ModbusClient, isCoil bool) {
	...
	var illegalDataAddressError *modbus.IllegalDataAddressError
	var illegalFunctionError *modbus.IllegalFunctionError
...
func performPing(client *modbus.ModbusClient, count uint16, interval time.Duration) {
	var err error
	var countReply, countTimeout bool
	var replyCount, timeoutCount, otherErrCount uint
	var start, intermediate time.Time
	var rtt, minRTT, maxRTT, sumRTT time.Duration
	...
	var illegalDataAddressError *modbus.IllegalDataAddressError
	var illegalFunctionError *modbus.IllegalFunctionError
	var requestTimedOutError *modbus.RequestTimedOutError
	var gatewayTargetFailedToRespondError *modbus.GatewayTargetFailedToRespondError
	for run := uint16(0); run < count; run++ {
		intermediate = time.Now()
		_, err = client.ReadRegister(0x0000, modbus.HOLDING_REGISTER)

		rtt = time.Since(intermediate)
		sumRTT += rtt
		if run == 0 || rtt < minRTT {
			minRTT = rtt
		}
		...
		if err == nil {
			...
			if errors.As(err, &requestTimedOutError) {
				countTimeout = true
			}
			if errors.As(err, &gatewayTargetFailedToRespondError) {
				countTimeout = true
			}
		}
		if countReply {
	...
	fmt.Printf("--- ping statistics ---\n"+
		...
		count, replyCount, otherErrCount, timeoutCount,
		time.Since(start).Round(time.Millisecond))

	fmt.Printf("rtt min/avg/max = %v/%v/%v\n",
		minRTT.Round(time.Microsecond),
		(sumRTT / time.Duration(count)).Round(time.Microsecond),
