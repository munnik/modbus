...
func TestTCPServerCoilsAndDiscreteInputs(t *testing.T) {
	...
	var illegalDataAddressError *IllegalDataAddressError
	if !errors.As(err, &illegalDataAddressError) {
		t.Errorf("expected IllegalDataAddressError, got: %v", err)
	}
	...
	if !errors.As(err, &illegalFunctionError) {
		t.Errorf("client.WriteCoils() should have returned IllegalFunctionError, got: %v", err)
	}
	err = client.WriteCoil(0x0005, false)
	if !errors.As(err, &illegalFunctionError) {
		t.Errorf("client.WriteCoil() should have returned IllegalFunctionError, got: %v", err)
	}
	_, err = client.ReadCoils(0x0005, 1)
	if !errors.As(err, &illegalFunctionError) {
		t.Errorf("client.ReadCoils() should have returned IllegalFunctionError, got: %v", err)
	}
	_, err = client.ReadDiscreteInputs(0x0005, 1)
	if !errors.As(err, &illegalFunctionError) {
		t.Errorf("client.ReadDiscreteInputs() should have returned IllegalFunctionError, got: %v", err)
	}
...
func TestTCPServerHoldingAndInputRegisters(t *testing.T) {
	...
	if !errors.As(err, &illegalDataAddressError) {
		t.Errorf("client.ReadRegisters() should have returned IllegalDataAddressError, got: %v", err)
	}
	...
	var illegalFunctionError *IllegalFunctionError
	if !errors.As(err, &illegalFunctionError) {
		t.Errorf("client.WriteRegisters() should have returned IllegalFunctionError, got: %v", err)
	}
	err = client.WriteRegister(0x0001, 0xffff)
	if !errors.As(err, &illegalFunctionError) {
		t.Errorf("client.WriteRegister() should have returned IllegalFunctionError, got: %v", err)
	}
	_, err = client.ReadRegisters(0x0005, 1, HOLDING_REGISTER)
	if !errors.As(err, &illegalFunctionError) {
		t.Errorf("client.ReadRegisters() should have returned IllegalFunctionError, got: %v", err)
	}
...
func (th *tcpTestHandler) HandleCoils(req *CoilsRequest) (res []bool, err error) {
	if req.UnitId != 9 {
		...
		err = NewIllegalFunctionError("unit is is not 9")
		return
	}
...
func (th *tcpTestHandler) HandleDiscreteInputs(req *DiscreteInputsRequest) (res []bool, err error) {
	...
	if req.Addr+req.Quantity > uint16(len(th.di)) {
		err = NewIllegalDataAddressError("address (%d) is out of range (%d)", req.Addr+req.Quantity, uint16(len(th.di)))
		return
	}
...
func (th *tcpTestHandler) HandleHoldingRegisters(req *HoldingRegistersRequest) (res []uint16, err error) {
	...
	if req.Addr+req.Quantity > uint16(len(th.holding)) {
		err = NewIllegalDataAddressError("address (%d) is out of range (%d)", req.Addr+req.Quantity, uint16(len(th.holding)))
		return
	}
...
func (th *tcpTestHandler) HandleInputRegisters(req *InputRegistersRequest) (res []uint16, err error) {
	...
	if req.Addr+req.Quantity > uint16(len(th.input)) {
		err = NewIllegalDataAddressError("address (%d) is out of range (%d)", req.Addr+req.Quantity, uint16(len(th.input)))
		return
	}
