...
func (rt *rtuTransport) ExecuteRequest(req *pdu) (res *pdu, err error) {
	...
	var badCRCError *BadCRCError
	var protocolError *ProtocolError
	var shortFrameError *ShortFrameError
	if errors.As(err, &badCRCError) || errors.As(err, &protocolError) || errors.As(err, &shortFrameError) {
...
func (rt *rtuTransport) readRTUFrame() (res *pdu, err error) {
	...
	if (byteCount > 0 || err == nil) && byteCount != 3 {
		err = NewShortFrameError("expected byte count (3) doesn't match the byte count (%d) read", byteCount)
		return
	}
	...
	if byteCount+bytesNeeded > maxRTUFrameLength {
		err = NewProtocolError("read more (%d) than the max RTU frame length (%d)", byteCount+bytesNeeded, maxRTUFrameLength)
		return
	}
	...
	if byteCount != bytesNeeded {
		err = NewShortFrameError("expected %v bytes, received %v", bytesNeeded, byteCount)
		return
	}
	...
	if !crc.isEqual(rxbuf[3+bytesNeeded-2], rxbuf[3+bytesNeeded-1]) {
		err = NewBadCRCError("incorrect CRC %d for received data %v", crc, rxbuf)
		return
	}
...
func expectedResponseLenth(responseCode uint8, responseLength uint8) (byteCount int, err error) {
	...
	case fcReadHoldingRegisters | 0x80,
		...
		byteCount = 0
	default:
		err = NewProtocolError("unknown response code %d", responseCode)
	}
