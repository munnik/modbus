...
func NewClient(conf *ClientConfiguration) (mc *ModbusClient, err error) {
	...
	case "tcp+tls":
		...
		if mc.conf.TLSClientCert == nil {
			err = NewConfigurationError("missing client certificate")
			return
		}
		...
		if mc.conf.TLSRootCAs == nil {
			err = NewConfigurationError("missing CA/server certificate")
			return
		}
	...
	default:
		if len(splitURL) != 2 {
			err = NewConfigurationError("missing CA/server certificate")
			err = NewConfigurationError("missing client type in URL '%s'", mc.conf.URL)
		} else {
			err = NewConfigurationError("unsupported client type '%s'", clientType)
		}
		return
	}
...
func (mc *ModbusClient) Open() (err error) {
	...
	default:
		// should never happen
		err = NewConfigurationError("unkown transport type %v", mc.transportType)
	}
...
func (mc *ModbusClient) SetEncoding(endianness Endianness, wordOrder WordOrder) (err error) {
	...
	if endianness != BIG_ENDIAN && endianness != LITTLE_ENDIAN {
		err = NewUnexpectedParametersError("unknown endianness value %v", endianness)
		return
	}

	if wordOrder != HIGH_WORD_FIRST && wordOrder != LOW_WORD_FIRST {
		err = NewUnexpectedParametersError("unknown word order value %v", wordOrder)
		return
	}
...
func (mc *ModbusClient) WriteCoil(addr uint16, value bool) (err error) {
	...
	case res.functionCode == req.functionCode:
		...
		if len(res.payload) != 4 ||
			...
			(value && res.payload[2] != 0xff) || res.payload[3] != 0x00 {
			err = NewProtocolError("unexpected payload %v", res.payload)
			return
		}

	case res.functionCode == (req.functionCode | 0x80):
		if len(res.payload) != 1 {
			err = NewProtocolError("the length of the payload (%d) is not the expected length (%d)", len(res.payload), 1)
			return
		}

		err = mapExceptionCodeToError(res.payload[0], "unexpected response code (%d) for request code (%d)", res.functionCode, req.functionCode)

	default:
		err = NewProtocolError("unexpected response code (%d) for request code (%d)", res.functionCode, req.functionCode)
	}
...
func (mc *ModbusClient) WriteCoils(addr uint16, values []bool) (err error) {
	...
	if quantity == 0 {
		err = NewUnexpectedParametersError("quantity of coils is 0")
		mc.logger.Error("quantity of coils is 0")
		return
	}

	if quantity > 0x7b0 {
		err = NewUnexpectedParametersError("quantity of coils exceeds 1968")
		return
	}

	if uint32(addr)+uint32(quantity)-1 > 0xffff {
		err = NewUnexpectedParametersError("end coil address is past 0xffff")
		return
	}
...
func (mc *ModbusClient) readBools(addr uint16, quantity uint16, di bool) (values []bool, err error) {
	...
	if quantity == 0 {
		err = NewUnexpectedParametersError("quantity of coils/discrete inputs is 0")
		return
	}

	if quantity > 2000 {
		err = NewUnexpectedParametersError("quantity of coils/discrete inputs exceeds 2000")
		return
	}

	if uint32(addr)+uint32(quantity)-1 > 0xffff {
		err = NewUnexpectedParametersError("end coil/discrete input address is past 0xffff")
		return
	}
	...
	case res.functionCode == req.functionCode:
		...
		if len(res.payload) != expectedLen {
			err = NewProtocolError("the length of the payload (%d) is not the expected length (%d)", len(res.payload), expectedLen)
			return
		}

		// validate the byte count field
		if int(res.payload[0])+1 != expectedLen {
			err = NewProtocolError("the byte count field of the payload (%d) doesn't equal the expected length (%d)", int(res.payload[0])+1, expectedLen)
			return
		}
...
func (mc *ModbusClient) readRegisters(addr uint16, quantity uint16, regType RegType) (bytes []byte, err error) {
	...
	default:
		err = NewUnexpectedParametersError("unexpected register type (%v)", regType)
		return
	}

	if quantity == 0 {
		err = NewUnexpectedParametersError("quantity of registers is 0")
		return
	}
	if quantity > 123 {
		err = NewUnexpectedParametersError("quantity of registers exceeds 123")
		return
	}

	if uint32(addr)+uint32(quantity)-1 > 0xffff {
		err = NewUnexpectedParametersError("end register address is past 0xffff")
		return
	}
	...
	case res.functionCode == req.functionCode:
		...
		if len(res.payload) != 1+2*int(quantity) {
			err = NewProtocolError("the length of the payload (%d) is not the expected length (%d)", len(res.payload), 1+2*int(quantity))
			return
		}
		...
		if uint(res.payload[0]) != 2*uint(quantity) {
			err = NewProtocolError("the byte count field of the payload (%d) doesn't equal the expected length (%d)", uint(res.payload[0]), 2*uint(quantity))
			return
		}
...
func (mc *ModbusClient) executeRequest(req *pdu) (res *pdu, err error) {
	...
	if err != nil {
		...
		if os.IsTimeout(err) {
			err = NewRequestTimedOutError(err.Error())
		}
		return
	}
	...
	if (res.functionCode&0x80) == 0x00 && res.unitId != req.unitId {
		err = NewBadUnitIdError("the unit id of the response (%d) doesn't match the unit id of the request (%d)", res.unitId, req.unitId)
		return
	}
	// accept errors from gateway devices (using special unit id #255)
	if (res.functionCode&0x80) == 0x80 && res.unitId != req.unitId && res.unitId != 0xff {
		err = NewBadUnitIdError("the unit id of the response (%d) doesn't match the unit id of the request (%d) and the response unit id is not 0xff", res.unitId, req.unitId)
		return
	}
